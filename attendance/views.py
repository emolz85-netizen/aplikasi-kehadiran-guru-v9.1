...
from .models import Attendance, LeaveRequest, OfficialDuty, TeacherProfile, SchoolSettings, SchoolHoliday, AccountActivity, PasswordRecoveryRequest, PushSubscription, AppNotification, TrustedDevice, LocationSecurityEvent, FaceLoginAttempt, VapidConfiguration
...
const PRECACHE = [OFFLINE_URL,"/login/","/pwa/pasang/","/manifest.json","/static/attendance/style.css","/static/attendance/app.js","/static/attendance/pwa.js","/static/attendance/push.js","/static/attendance/mobile.js","/static/attendance/icon-192.png","/static/attendance/icon-512.png","/static/attendance/icon-maskable-192.png","/static/attendance/icon-maskable-512.png","/static/attendance/apple-touch-icon.png"];
self.addEventListener("install",e=>e.waitUntil(caches.open(STATIC_CACHE).then(c=>c.addAll(PRECACHE))));
...
self.addEventListener("push",e=>{let d={title:"Sistem Kehadiran Guru",body:"Anda menerima notifikasi baharu.",url:"/notifikasi/",category:"SISTEM"};try{d=Object.assign(d,e.data.json());}catch(x){}const options={body:d.body,icon:"/static/attendance/icon-192.png",badge:"/static/attendance/icon-192.png",image:d.image||undefined,data:{url:d.url||"/notifikasi/",notification_id:d.notification_id},tag:"kehadiran-"+(d.notification_id||d.category||Date.now()),renotify:true,requireInteraction:d.category==="KESELAMATAN",silent:false,vibrate:[180,80,180],timestamp:Date.now(),actions:[{action:"open",title:"Buka"},{action:"dismiss",title:"Tutup"}]};e.waitUntil(self.registration.showNotification(d.title,options).then(()=>self.registration.setAppBadge?self.registration.setAppBadge(1):null));});
self.addEventListener("notificationclick",e=>{e.notification.close();if(e.action==="dismiss")return;const t=e.notification.data&&e.notification.data.url?e.notification.data.url:"/notifikasi/";e.waitUntil(clients.matchAll({type:"window",includeUncontrolled:true}).then(list=>{for(const c of list){if("navigate" in c)c.navigate(t);if("focus" in c)return c.focus();}return clients.openWindow?clients.openWindow(t):null;}));});
...
def school_management_center(request):
    ...
    school = SchoolSettings.load()
    role = get_user_role(request.user)
    vapid = VapidConfiguration.objects.first()
    return render(request, "attendance/school_management_center.html", {
        ...
        # Direct module destinations: clicking a card opens the selected module
        # immediately instead of stopping at an intermediate settings dashboard.
        "school_profile_url": reverse("admin:attendance_schoolsettings_change", args=[school.pk]),
        "working_session_url": reverse("admin:attendance_schoolsettings_change", args=[school.pk]),
        "system_configuration_url": reverse("admin:attendance_schoolsettings_change", args=[school.pk]),
        "communication_url": (
            reverse("admin:attendance_vapidconfiguration_change", args=[vapid.pk])
            if vapid else reverse("admin:attendance_vapidconfiguration_add")
        ),
    })
